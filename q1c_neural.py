...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    gradCE = y_hat - labels  # y_hat-y
    gradW2 = h.T.dot(gradCE)  # h.T *(y_hat - y)
    gradb2 = np.sum(gradCE, axis=0)  # sum(y_hat -y)
    gradH = gradCE.dot(W2.T)  # (y_hat -y)*W2.T
    gradZ = gradH * sigmoid_grad(h)  # (y_hat -y)*W2.T * sig(z)* (1 - sig(z))
    gradW1 = data.T.dot(gradZ)  # data.T* (y_hat - y) * W2.T* sig(z) * (1 - sig(z))
    gradb1 = np.sum(gradZ, axis=0)  # sum((y_hat-y) *W2.T * sig(z) *(1 - sig(z)))
    ### END YOUR CODE
