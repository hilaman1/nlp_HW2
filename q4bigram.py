...
def load_data_as_sentences(path, word_to_num):
    ...
    Conv:erts the training data to an array of integer arrays.
      ...
      returns:
        ...
        integer is a word.
    """
    file = open(path, 'r', encoding='ascii', errors='ignore')
    file2 = open('ascii.txt', 'w', encoding='ascii', errors='ignore')
    for line in file:
        file2.write(line)
    file.close()
    file2.close()
...
def lm_wrapper(in_word_index, out_word_index, num_to_word_embedding, dimensions, params):
    ...
    for i in range(BATCH_SIZE):
        index = random.randint(0, len(in_word_index) - 1)
        data[i] = num_to_word_embedding[in_word_index[index]]
        labels[i] = int_to_one_hot(out_word_index[index], output_dim)
    
    cost, grad = forward_backward_prop(data, labels, params, dimensions) # forward and backward prop
    ### END YOUR CODE
...
def eval_neural_lm(eval_data_path):
    ...
    perplexity = 0
    ### YOUR CODE HERE
    print("num examples = ", num_of_examples)
    for i in range(num_of_examples):
        if i % 10000 == 0:
            print(f"evaluating {i}th example")
...
data1 = r"shakespeare_for_perplexity.txt"
data2 = r"wikipedia_for_perplexity.txt"
...
if __name__ == "__main__":
    ...
    if os.path.exists(data1):
        perplexity = eval_neural_lm(data2)
